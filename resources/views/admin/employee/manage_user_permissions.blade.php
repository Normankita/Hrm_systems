                        ...
                        @foreach ($grouped as $groupName => $loopPermissions)
                            <div class="permission-group">
                                ...
                                @foreach ($loopPermissions as $permission)
                                    <label
                                        class="permission-box {{ $employee->user->hasPermissionTo($permission->name) ? 'selected' : '' }}">
                                        <div>
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}"
                                                {{ $employee->user->hasPermissionTo($permission->name) ? 'checked' : '' }}>
                                            {{ $permission->name }}
            ...
            $('#permissionsForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        window.location.href = "{{ route('admin.employees.edit.permissions',$employee->id) }}";
                    },
                    error: function(xhr) {
