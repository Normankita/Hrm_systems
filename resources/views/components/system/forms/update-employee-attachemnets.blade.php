...
<div class="row">
    <x-system.form-inputs.file-upload name="passport_photo" label="Passport Photo" accept="image/jpeg,image/png,image/jpg"
        ...
        @endphp
        @if ($attachmentName)
            <x-system.attachment-file-icon :path="$attachment->path"
            type="image" :attachmentName="$attachmentName" />
        @endif
    ...
    <x-system.form-inputs.file-upload name="tin_document" label="TIN Document" accept="application/pdf" maxSize="5"
        ...
        @if ($attachmentName)
            <x-system.attachment-file-icon :path="$attachment->path"
                type="pdf" :attachmentName="$attachmentName" />
        @endif
    </x-system.form-inputs.file-upload>
